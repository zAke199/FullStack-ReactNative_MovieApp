...
const MovieDetails = () => {
    ...
    return (
        <View className="bg-[#030014] flex-1">
            ...
            >
                ...
                <View className="flex-col items-start justify-center mt-5 px-5">
                    ...
                    <View className="flex-row items-center gap-x-1 mt-2">
                        <Text className="text-[#9CA4AB] text-sm">
                    ...
                    <View className="flex flex-row justify-between w-1/2">
                        {movie?.budget ? (
                            ...
                            <MovieInfo label="Budget" value="Not available" />
                        )}
                        {movie?.revenue ? (
                            <MovieInfo
                                label="Revenue"
                                value={`$${(movie?.revenue / 1_000_000).toFixed(2)} Million`}
                            />
                        ) : (
                            <MovieInfo label="Revenue" value="Not applicable" />
                        )}
                    </View>
                    <MovieInfo
            ...
            <TouchableOpacity
                className="absolute bottom-5 left-0 right-0 mx-5 bg-[#AB8BFF] rounded-lg py-3.5 flex flex-row items-center justify-center z-50"
                onPress={router.back}
            >
                <Image
                    source={icons.arrow}
